...
export async function GET(req: Request) {
  ...
  const target = new URL(`${base}/api/promo-codes`);

  // forward ALL query params (q, page, game/game_slug, product_id, payment_method_id, dll)
  ...
  if (!res.ok) {
    return NextResponse.json({ error: "Failed to fetch promo codes", raw: data }, { status: res.status });
  }
