...
export async function GET(req: Request) {
  ...
  const target = new URL(`${base}/api/invoices/search-by-whatsapp`);
  ...
  if (!res.ok) {
    return NextResponse.json({ error: "Failed to search invoices by whatsapp", raw: data }, { status: res.status });
  }
